...
public class MagicianNet_Ctrl : Photon.MonoBehaviour {
	...
	public class Weapons
	{
		...
		public GameObject torch_Holster = null;
	}
	...
	void Awake() {
		...
		weaponNames = new string[]{"Sword","Bow","Torch"};
		renderersSet = new Renderer[,]{
			{weapons.sword_Holster.renderer,weapons.torch_Holster.renderer},
			{weapons.bow_Holster.renderer,weapons.sword_Holster.renderer},
			{weapons.torch_Holster.renderer,weapons.bow_Holster.renderer}
		};
	...
	public void setWeaponRenderersRPC(int weaponStateInInt) {
		...
		if(weapons.torch_Holster) weapons.torch_Holster.renderer.enabled = weaponRendererSettings[weaponStateInInt,6];
	...
	IEnumerator JustHolster ()
	{
		...
		{
			yield return StartCoroutine(DrawHolster(0.3f,weapons.sword_Holster.renderer, weapons.sword_Holster.renderer));
			a.SetBool("Sword", false);
			yield break;
		}
		else if(weaponState == WeaponState.Bow)
		{
			yield return StartCoroutine(DrawHolster(0.6f,weapons.bow_Holster.renderer, weapons.bow_Holster.renderer));
			a.SetBool("Bow", false);
			yield break;
		}
		else if(weaponState == WeaponState.Torch)
		{
			yield return StartCoroutine(DrawHolster(0.9f,weapons.torch_Holster.renderer, weapons.torch_Holster.renderer));
		...
		if(baseState == BaseState.Base)
		{
			...
			if(weaponState == WeaponState.Sword) // Skip if no weapon is assigned
			{
				...
				else
				{
					...
					if(weapons.torch_Holster) weapons.torch_Holster.renderer.enabled = false;
				}
			}
			...
			if(weaponState == WeaponState.Bow) // Skip if no weapon is assigned
			{
				...
				{
					if(weapons.sword_Holster) weapons.sword_Holster.renderer.enabled = false; // On
					if(weapons.bow_Holster) weapons.bow_Holster.renderer.enabled = true;
					if(weapons.torch_Holster) weapons.torch_Holster.renderer.enabled = false;
				}
			}
			if(weaponState == WeaponState.Torch) // Skip if no weapon is assigned
			{
				if(weapons.torch_Holster == null) // No weapon - next
				{
					weaponState += 1; 
				}
				else
				{
					if(weapons.sword_Holster) weapons.sword_Holster.renderer.enabled = false; // On
					if(weapons.bow_Holster) weapons.bow_Holster.renderer.enabled = false;
					if(weapons.torch_Holster) weapons.torch_Holster.renderer.enabled = true;
				}
			}
