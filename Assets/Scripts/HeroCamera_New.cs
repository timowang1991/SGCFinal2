...
public class HeroCamera_New : MonoBehaviour
{
	public LayerMask collisionLayers = -1;
    public float heroHeight = 12.0f;
    public float heroDistance = 15.0f;
	...
	private CameraState oldCamState;
	...
	{
		if (isInside) {
			this.GetComponent<CtrlThings>().ChangeToCatapult(gameobject);
			oldCamState = camState;
			camState = CameraState.Catapults;
			//Debug.Log("Catapults:" + gameobject.name);
			ThingToControl = gameobject;

		} else {
			camState = oldCamState;
		}
	...
	void Catapults()
	{
		...
		cam.rotation = camRot;
		cam.position = ThingToControl.transform.Find("CamToPut").transform.position;
	}
	//self add end
    ...
    void Start ()
    {
		...
		hCtrl = GetComponent <HeroCtrlAlias>();
    ...
    void LateUpdate ()
    {
		...
		case CameraState.Orbit:
			Orbit();
			break;
		case CameraState.Catapults://add state
			Catapults();
			break;
		}
	}
